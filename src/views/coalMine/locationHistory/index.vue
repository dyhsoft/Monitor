            ...
            <el-card shadow="hover">
                <el-form :inline="true">
                    <el-form-item label="姓名">
                        <el-input v-model="state.personName" placeholder="请输入" clearable style="width: 150px;" />
                    </el-form-item>
...
async function loadData() {
    ...
    state.loading = true;
    try {
        const res = await getAPI(PersonApi).getRealtimePage(state.queryParams);
        state.tableData = res.data.result?.items || [];
    } catch (error) {
        console.error('加载人员历史数据失败:', error);
        state.tableData = [];
    } finally {
        state.loading = false;
    }
