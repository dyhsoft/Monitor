...
public class CoalMineAreaService : IDynamicApiController, ITransient
{
    ...
    /// 从数据文件夹自动导入煤矿
    /// </summary>
    /// <param name="folders">要扫描的文件夹路径（可选，不填则使用默认工作文件夹）</param>
    /// <returns>导入数量</returns>
    [HttpPost]
    [DisplayName("自动导入煤矿")]
    public async Task<int> AutoImportCoalMines([FromQuery] List<string> folders)
    {
        var autoImportService = App.GetService<CoalMineAutoImportService>();
        
        if (folders == null || folders.Count == 0)
        {
            // 使用默认工作文件夹
            return await autoImportService.AutoImportFromWorkFolders();
        }
        
        return await autoImportService.AutoImportFromFolders(folders);
    }

    /// <summary>
    /// 初始化数据库列（用于SQLite手动添加列）
    /// </summary>
    [HttpGet]
    [DisplayName("初始化数据库列")]
    public async Task<string> InitDbColumns()
    {
        try
        {
            var db = App.GetService<SqlSugarScope>();
            // 检查列是否存在，不存在则添加
